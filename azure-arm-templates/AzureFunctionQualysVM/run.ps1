<#  
    Version:        1.0
    Author(s):      John Bailey, Chi Duong
	...
	$kbytes = ([System.Text.Encoding]::UTF8.GetBytes($json)).Count/1024         
    	...
    	if ($kbytes -gt 30){                                                                                           
	        ...
	        ForEach ($QID in $_.Detections){
	            ...
	            # Add it to the new object array that contains all the records parent information / individual detection
	            $newcustomObjects += $newObject
        	}
        	$newcustomObjects | ForEach-Object {
            # convert each parent / indvidual detection to JSON and API POST to Log Analytics Workspace as its own record                                                                      
        	$json = $_ | ConvertTo-Json -Compress -Depth 3                                                            
    	...
    	# The record is less than 30kb, convert to JSON and send API POST to Log Analytics Workspace
    	$json = $_ | ConvertTo-Json -Compress -Depth 3         
        Post-LogAnalyticsData -customerId $customerId -sharedKey $sharedKey -body ([System.Text.Encoding]::UTF8.GetBytes($json)) -logType $TableName
        }
    }
